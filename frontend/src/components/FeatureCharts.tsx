...
interface Props {
  frames: Frame[];
  phases: Phase[];
  labels: { attention: string; movement: string; blink: string; visibility: string; tracking: string; away: string; phases: string };
}
...
export default function FeatureCharts({ frames, phases, labels }: Props) {
  ...
  return (
    <div className="card-glass p-5 mt-4">
      ...
      {phases.length > 0 && <div className="mt-6"><p className="text-xs uppercase tracking-widest text-ui-subtle mb-3">{labels.phases}</p><div className="space-y-2">{phases.map((phase) => { const ratio = Math.max(0, Math.min(1, Number(phase.attention_ratio ?? 0))); return <div key={String(phase.phase)} className="flex items-center gap-3 text-xs"><span className="w-28 truncate text-ui-muted">{String(phase.phase)}</span><div className="flex-1 h-2 rounded bg-slate-700/40"><div className="h-full rounded bg-teal-500" style={{ width: `${ratio * 100}%` }} /></div><span className="w-8 text-right text-ui-muted">{(ratio * 100).toFixed(0)}%</span></div>; })}</div></div>}
    </div>
  );
}
